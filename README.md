...
- **Owned overlay.** The injected `<script>` is ~150 lines of vanilla JS written for this crate — it is **not** a port of ORACEL's `chrome-client.js` / `artifact-sdk.js` (the harness content scanner flagged those modules as prompt-injection-shaped). The overlay contains no `http://` / `https://` / external-`src=` references and POSTs only to the relative `/feedback` path.
...
- **Reference acknowledgement.** The MIT-licensed [`kunchenguid/ORACEL`](https://github.com/kunchenguid/ORACEL) repository was read as a user-authorized architectural blueprint (Copyright (c) 2026 Kun Chen). No code was copied. See [ADR 0017](docs/adr/0017-vesper-lens-native-oracle.md) for the full contract.
