...
In Zed, typing `/` in the composer surfaces **40 implemented commands** (advertised via ACP `available_commands_update` on every new/load/resume): the frozen 28-command compatibility catalog plus 12 shared host-neutral extensions (`/remember`, `/recall`, `/forget`, `/memories`, `/promote`, `/demote`, `/embedding`, `/reasoning`, `/repository`, `/meta-learning`, `/observability`, `/journey`). Report commands execute in-process with no model call, `/max-iterations [N]` tunes the per-turn tool cap for the session, and unknown `/` text gets the oracle's bounded fallback. Slash turns never reach the provider and are never persisted. Safe report commands — including `/usage` — can be used while an agent turn is running without cancelling that work. Every host-owned command is wired with TUI parity: `/checkpoint`/`/rollback`/`/undo`/`/export`/`/sessions`/`/lineage`/`/ci`/`/plugins`/`/mcp` run against the same durable checkpoint/MCP stores the TUI uses, `/compact`/`/clear-history`/`/clear-plan` mutate the engine's own conversation state, `/usage` queries the active session provider's quota integration, and `/diff`/`/release` drive a real agent workflow turn. `/skills` reads the project layer **plus the cross-project global layer** (`~/.agent-vesper/memory`), so globally learned skills appear exactly like in the TUI.
