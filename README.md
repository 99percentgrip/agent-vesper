...
Agent Vesper is a **pure-Rust** AI agent harness that **remembers you across conversations**. It provides a provider-neutral runtime where any LLM provider can plug in — it ships with Z.ai GLM, LM Studio, and native OpenAI (API-key or ChatGPT subscription sign-in, with no Codex installation). The agent learns your preferences, tracks your projects, and recalls relevant context before every reply — all running locally with zero external services.
...
The chat footer carries the full multi-provider selector surface (advertised via ACP `sessionConfigOptions` on `session/new`/`load`/`resume`/`set`): **Provider** (TUI `/provider` parity — lists every registered adapter with live credential status; Z.ai GLM, LM Studio, and OpenAI are registered in every boot; switching swaps the acting provider for the next turn, GLM keeps its overrides across round trips, and unauthenticated GLM descriptions tell the user to run `--setup`) plus the controls of the **acting provider only** (dynamic capability gating, `docs/provider-capability-gating-prd.md`): when Z.ai GLM acts — **Model** (Mixture-of-Agents picker first, then the plan's models), **Reasoning** (Off/Standard, Deep·High/Deep·Max only on deep-reasoning models), **API Plan**, **Generation Style**, **Auxiliary Model**, **Mixture of Agents**; when LM Studio acts — ONLY a truthful **Model** picker fed by the server's live `/api/v1/models` catalog (verified LM Studio schema: live model ids, advertised context sizes; the pinned settings model always present as the offline fallback). **Permissions** (ask/bypass/read) is always advertised. GLM-only selections made while LM Studio acts are rejected fail-closed (never a silent cross-provider route). Every selection is validated against the provider's own surface (invented values are rejected fail-closed) and takes effect on the very next turn; the **token counter** follows the acting provider — GLM's frozen per-model context windows, the LM Studio model's advertised `max_context_length` (conservative floor when unadvertised, never GLM's 1M) — through live `usage_update` notifications.

In Zed, typing `/` in the composer surfaces **45 implemented commands** (advertised via ACP `available_commands_update` on every new/load/resume): the frozen 28-command compatibility catalog plus 17 shared host-neutral extensions (`/remember`, `/recall`, `/forget`, `/memories`, `/promote`, `/demote`, `/embedding`, `/reasoning`, `/repository`, `/meta-learning`, `/observability`, `/journey`, `/skill`, `/firewall`, `/sandbox`, `/daemon`, `/watch`). Report commands execute in-process with no model call, `/max-iterations [N]` tunes the per-turn tool cap for the session, and unknown `/` text gets the oracle's bounded fallback. `/skill <name|bundle:name> [task]` explicitly activates a skill workflow; ordinary prompts also use the shared provider-neutral router, which ranks bounded metadata, injects selected inline instructions only for the current request, and keeps isolated skill bodies in a bounded worker. `/firewall` reports the VRO-13 hard-denial firewall status (view only — disable requires a restart with `AGENT_VESPER_FIREWALL=off`); the firewall itself applies to shell-class tool calls regardless of permission mode, so Bypass never escapes the deny floor. Slash turns never reach the provider and are never persisted unless they intentionally drive a workflow. Safe report commands — including `/usage` — can be used while an agent turn is running without cancelling that work. Every host-owned command is wired with TUI parity: `/checkpoint`/`/rollback`/`/undo`/`/export`/`/sessions`/`/lineage`/`/ci`/`/plugins`/`/mcp` run against the same durable checkpoint/MCP stores the TUI uses, `/compact`/`/clear-history`/`/clear-plan` mutate the engine's own conversation state, `/usage` queries the active session provider's quota integration, and `/diff`/`/release` drive a real agent workflow turn. `/skills` reads the project layer **plus the cross-project global layer** (`~/.agent-vesper/memory`), so globally learned skills appear exactly like in the TUI.

Multi-step `update_plan` calls are published as native ACP `plan` updates, so
Zed displays the same live pending/in-progress/completed TODO state as the TUI.
...
Native OpenAI setup: `/settings` → Providers → OpenAI → API key or ChatGPT
subscription. No Codex installation is needed. See [the setup and capability
guide](docs/openai-provider.md) for account prerequisites and both-host usage.
...
3. **`/embedding` slash command (UX parity)** — registered in the Vesper-native surface:
   ...
   - `/embedding set source=lmstudio endpoint=... model=... api_key=... dimension=...` → parse, validate, persist `embedding.json`, replace the live adapter, and migrate stored vectors immediately in both TUI and ACP
