...
<tr><td><b>🧩 Reasoning Orchestrator</b></td><td>Strategy-driven orchestration (VRO): profile-driven routing across <code>Direct</code>, <code>GenerateVerifyRepair</code>, <code>ParallelCandidatesConsensus</code>, <code>ParallelCandidatesJudge</code>, and <code>ToolGroundedReact</code>. VRO-4 ships <b>parallel candidate branches</b> with strict isolation, deterministic ordering, budget-capped fan-out, and either quorum-based consensus (§11.4) or position-bias-shuffled judge arbitration (§11.5). VRO-5.1 ships a <b>tool-grounded ReAct loop</b> (§11.6) with Read-Before-Write policy enforcement, structured failure observations on tool errors, and an integrated permission sandbox. VRO-5.2 ships the production <b>LM Studio <code>ReactAgent</code></b> adapter — ReAct prompting contract, trajectory injection, and an infallible decision parser that maps model output to <code>CallTool</code>/<code>Finish</code> and feeds malformed JSON back as a structured observation for self-correction. Triggered automatically for trade-off, verification, and environment-evidence prompts.</td></tr>
</table>
...
```sh
AGENT_VESPER_VERSION=0.20.20 sh scripts/install.sh
```
...
```sh
cargo xtask verify          # fmt + clippy -D warnings + 799 tests + architecture
