...
The chat footer carries the full provider-routed selector surface (advertised via ACP `sessionConfigOptions` on `session/new`/`load`/`resume`/`set`): **Model** (Mixture-of-Agents picker first, then the active plan's models), **Reasoning** (Off/Standard, Deep·High/Deep·Max only on deep-reasoning models), **API Plan**, **Generation Style**, **Auxiliary Model**, **Mixture of Agents**, and **Permissions** (ask/bypass/read). Every selection is validated against the provider's own catalog (invented values are rejected fail-closed) and takes effect on the very next turn; the **token counter** follows the selected model's frozen context window through live `usage_update` notifications.
