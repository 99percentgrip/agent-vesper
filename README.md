...
In Zed, typing `/` in the composer surfaces **41 implemented commands** (advertised via ACP `available_commands_update` on every new/load/resume): the frozen 28-command compatibility catalog plus 13 shared host-neutral extensions (`/remember`, `/recall`, `/forget`, `/memories`, `/promote`, `/demote`, `/embedding`, `/reasoning`, `/repository`, `/meta-learning`, `/observability`, `/journey`, `/firewall`). Report commands execute in-process with no model call, `/max-iterations [N]` tunes the per-turn tool cap for the session, and unknown `/` text gets the oracle's bounded fallback. `/firewall` reports the VRO-13 hard-denial firewall status (view only — disable requires a restart with `AGENT_VESPER_FIREWALL=off`); the firewall itself applies to shell-class tool calls regardless of permission mode, so Bypass never escapes the deny floor. Slash turns never reach the provider and are never persisted. Safe report commands — including `/usage` — can be used while an agent turn is running without cancelling that work. Every host-owned command is wired with TUI parity: `/checkpoint`/`/rollback`/`/undo`/`/export`/`/sessions`/`/lineage`/`/ci`/`/plugins`/`/mcp` run against the same durable checkpoint/MCP stores the TUI uses, `/compact`/`/clear-history`/`/clear-plan` mutate the engine's own conversation state, `/usage` queries the active session provider's quota integration, and `/diff`/`/release` drive a real agent workflow turn. `/skills` reads the project layer **plus the cross-project global layer** (`~/.agent-vesper/memory`), so globally learned skills appear exactly like in the TUI.
