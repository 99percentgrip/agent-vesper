...
4. **VesperLens file-save interceptor.** `lens_integration.rs` gains `html_artifact_for_write_file(name, arguments)` and the `LensObservingInvoker<'a>` decorator. After every successful `write_file` to an `.html` path, the decorator routes the content through `LensReviewPort::review` and the React loop **halts** mid-turn for human review (the directive's "halt for human review" semantics). `Approve` returns the original tool result; `Reject` / `Modify` append the verdict so the model's next step can react. `VroOrchestrator::execute_react` wires the decorator transparently when `lens_port` is configured — zero-cost when not.
