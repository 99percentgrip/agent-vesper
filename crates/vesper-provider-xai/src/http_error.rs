...
pub(crate) async fn rejection(
    ...
    let (category, message) = match status {
        401 | 403 => (
            ...
            "xAI denied access; check authentication and model entitlement",
        ),
        429 => (
        ...
        400 | 404 | 422 => (
            ...
            "xAI rejected the request or selected model",
        ),
        _ => (ErrorCategory::Transport, "xAI service request failed"),
    };
    ...
    let payload = tokio::select! { biased;
        _ = async { while !cancel.is_cancelled() { tokio::time::sleep(Duration::from_millis(25)).await; } } => {
            let mut cancelled = error("xAI request cancelled", ErrorCategory::Cancellation, false);
    ...
    if matches!(result.http_status, Some(400 | 404 | 422)) {
        let message = match code {
            "context_length_exceeded" => {
                ...
                "xAI rejected the request: context window exceeded"
            }
            "invalid_image"
            | "invalid_image_format"
            | "invalid_base64_image"
            | "image_too_large" => {
                "xAI rejected image input; check image format, encoding and size"
            }
            "model_not_found" => {
                "xAI rejected the selected model; check model availability and account access"
            }
            "unsupported_parameter" => "xAI rejected an unsupported request parameter",
            "invalid_json_schema" => "xAI rejected the structured-output schema",
            _ => "xAI rejected a request field; see the safe provider error code",
        };
...
fn classify_parameter(result: &mut ProviderError, error_body: &Value) {
    ...
    ) = error_body["param"].as_str()
    {
        result
            ...
            .diagnostics
            .fields
            .insert("xai:parameter", serde_json::json!(param))
