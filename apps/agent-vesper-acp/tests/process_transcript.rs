...
fn stdio_transcript_reaches_real_glm_adapter_with_protocol_pure_stdout() {
    ...
    assert_eq!(
        ...
        Some(47 + usize::from(cfg!(feature = "swarm")) + usize::from(cfg!(feature = "bridge"))),
...
fn empty_prompt_and_slash_commands_never_dispatch_provider() {
    ...
    assert_eq!(
        advertised.len(),
        47 + usize::from(cfg!(feature = "swarm")) + usize::from(cfg!(feature = "bridge"))
    );
    assert_eq!(advertised.contains(&"swarm"), cfg!(feature = "swarm"));
    assert!(advertised.contains(&"web"));
    assert_eq!(
        advertised_first_and_last(&advertised),
        (
            "help",
            if cfg!(feature = "bridge") {
                "bridge"
            } else if cfg!(feature = "swarm") {
                "swarm"
            } else {
                "watch"
            }
