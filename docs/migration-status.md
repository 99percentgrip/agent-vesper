...
| Provider-neutral domain/runtime/agent harness | IMPLEMENTED | Typed provider registry, session supervisor, multi-turn agent loop, permission gate, progress port, hosted tool service, result-aware loop detection, truthful incomplete-terminal classification, four-segment bounded autonomous continuation while native-plan items remain open, explicit ultimate-cap outcomes, and transactional partial-history interruption outcomes shared by ACP/TUI |
| Z.ai GLM production adapter | IMPLEMENTED | Exact frozen six-model catalog, Coding/Standard/BigModel endpoints, auth, streaming, tools, reasoning, usage, image gating, distinct generation/inactivity/EOF/transport classification, bounded safe continuation before tool calls, and no-replay ambiguous-tool handling |
| ACP executable | IMPLEMENTED | Full harness is the production default; live ACP permission requests; persisted-session ports; setup/install lifecycle; native structured TODO/plan updates for Zed; actionable agent-loop failure classifications and preserved partial interruption results; 28-command oracle slash catalog advertised (new/load/resume) and executed in-process with fixture-exact `/help`, unknown-command fallback, per-session overrides, non-persisted slash turns, and streaming tool-call correlation |
