...
| VRO-14 web oracle extraction | PARTIAL | Pure perception and action data modules landed. The gap repair connects passive tools to the sandbox helper, fixes discovery, helper protocol, egress, robots, and output bounds. Production CDP browser sessions, render escalation, sitemap discovery, and the published pinned driver image remain open. `docs/foundation/vro14-gap-audit.md` is the current evidence and prerequisite record; historical phase test counts do not establish browser acceptance. |
