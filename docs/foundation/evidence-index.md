...
- [Critical tool-output stall permanent repair](2026-09-24-critical-tool-output-stall-repair.md) — reproduces the shared `run_command` pipe-backpressure defect red-first, replaces exit-before-read capture with concurrent bounded draining and owned-tree cleanup, verifies TUI/ACP recovery and Linux descendant settlement, and records provider-surface parity plus the still-unexecuted live tool-selection correlation comparison.
- [Tool-output stall platform verification preparation](2026-09-25-tool-output-stall-platform-verification.md) — finds the Windows-zero-test and PID-tree rediscovery gaps, replaces cleanup ownership with POSIX process groups/Windows Job Objects, makes the behavioral matrix portable and explicit in five-target CI, records local red→green evidence, and preserves Windows/macOS as NOT RUN because GitHub-hosted runners require the currently prohibited commit/push.
