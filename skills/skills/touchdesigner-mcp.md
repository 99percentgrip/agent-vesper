...
```
the agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "$HOME/.agent-vesper/memory/skills/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Register the `twozero_td` MCP server in the agent's MCP registry (if missing)
...
The table above covers the 32 tools used in typical creative workflows. The remaining 4 tools (`td_project_quit`, `td_test_session`, `td_dev_log`, `td_clear_dev_log`) are admin/dev-mode utilities — see `~/.agent-vesper/memory/skills/touchdesigner-mcp/references/mcp-tools.md` for the full 36-tool reference with complete parameter schemas.
...
See `~/.agent-vesper/memory/skills/touchdesigner-mcp/references/network-patterns.md` for complete build scripts + shader code.
...
| File | What |
|------|------|
| `~/.agent-vesper/memory/skills/touchdesigner-mcp/references/pitfalls.md` | Hard-won lessons from real sessions |
| `~/.agent-vesper/memory/skills/touchdesigner-mcp/references/operators.md` | All operator families with params and use cases |
| `~/.agent-vesper/memory/skills/touchdesigner-mcp/references/network-patterns.md` | Recipes: audio-reactive, generative, GLSL, instancing |
| `~/.agent-vesper/memory/skills/touchdesigner-mcp/references/mcp-tools.md` | Full twozero MCP tool parameter schemas |
| `~/.agent-vesper/memory/skills/touchdesigner-mcp/references/python-api.md` | TD Python: op(), scripting, extensions |
| `~/.agent-vesper/memory/skills/touchdesigner-mcp/references/troubleshooting.md` | Connection diagnostics, debugging |
| `~/.agent-vesper/memory/skills/touchdesigner-mcp/references/glsl.md` | GLSL uniforms, built-in functions, shader templates |
| `~/.agent-vesper/memory/skills/touchdesigner-mcp/references/postfx.md` | Post-FX: bloom, CRT, chromatic aberration, feedback glow |
| `~/.agent-vesper/memory/skills/touchdesigner-mcp/references/layout-compositor.md` | HUD layout patterns, panel grids, BSP-style layouts |
| `~/.agent-vesper/memory/skills/touchdesigner-mcp/references/operator-tips.md` | Wireframe rendering, feedback TOP setup |
| `~/.agent-vesper/memory/skills/touchdesigner-mcp/references/geometry-comp.md` | Geometry COMP: instancing, POP vs SOP, morphing |
| `~/.agent-vesper/memory/skills/touchdesigner-mcp/references/audio-reactive.md` | Audio band extraction, beat detection, envelope following |
| `~/.agent-vesper/memory/skills/touchdesigner-mcp/references/animation.md` | LFOs, timers, keyframes, easing, expression-driven motion |
| `~/.agent-vesper/memory/skills/touchdesigner-mcp/references/midi-osc.md` | MIDI/OSC controllers, TouchOSC, multi-machine sync |
| `~/.agent-vesper/memory/skills/touchdesigner-mcp/references/particles.md` | POPs and legacy particleSOP — emission, forces, collisions |
| `~/.agent-vesper/memory/skills/touchdesigner-mcp/references/projection-mapping.md` | Multi-window output, corner pin, mesh warp, edge blending |
| `~/.agent-vesper/memory/skills/touchdesigner-mcp/references/external-data.md` | HTTP, WebSocket, MQTT, Serial, TCP, webserverDAT |
| `~/.agent-vesper/memory/skills/touchdesigner-mcp/references/panel-ui.md` | Custom params, panel COMPs, button/slider/field, panelExecuteDAT |
| `~/.agent-vesper/memory/skills/touchdesigner-mcp/references/replicator.md` | replicatorCOMP — data-driven cloning, layouts, callbacks |
| `~/.agent-vesper/memory/skills/touchdesigner-mcp/references/dat-scripting.md` | Execute DAT family — chop/dat/parameter/panel/op/executeDAT |
| `~/.agent-vesper/memory/skills/touchdesigner-mcp/references/3d-scene.md` | Lighting rigs, shadows, IBL/cubemaps, multi-camera, PBR |
| `~/.agent-vesper/memory/skills/touchdesigner-mcp/scripts/setup.sh` | Automated setup script |

---

> You're not writing code. You're conducting light.

> Bundled resources for this skill (~/.agent-vesper/memory/skills/touchdesigner-mcp/references/scripts/templates) live in `~/.agent-vesper/memory/skills/touchdesigner-mcp/` next to this file.
