...
version: 1.1.3
author: xdevplatform + openclaw + Agent Vesper library
license: MIT
...
- The user must fill `~/.xurl` with secrets manually on their own machine. In Docker, this must be the `~` seen by agent tool subprocesses; see the Docker note below.
...
> **Docker HOME pitfall:** inside containers, the user shell's `HOME` and the agent subprocess `HOME` may differ, so `~/.xurl` can resolve differently for agent-run `xurl` commands. Run the user setup with the same `HOME` the agent's `bash` tool uses:
> ```bash
...
> ```
> If `HOME=/opt/data xurl auth status` succeeds but `HOME=/opt/data/home xurl auth status` shows no apps or tokens, agent tool calls will not see the credentials.
...
- **Token storage:** `~/.xurl` is YAML. In Docker, use the agent subprocess HOME (`/opt/data/home` in the official image) so tokens land under `/opt/data/home/.xurl`. Never read or send this file to LLM context.
...
- Library adaptation: reformatted for the agent skill conventions; safety guardrails preserved verbatim.
