...
related_skills: [xlsx, powerpoint]
---
...
`bash` tool; each supports `--help` and prints JSON to stdout.

```bash
python ~/.agent-vesper/memory/skills/docx/scripts/docx_create.py spec.json out.docx
python ~/.agent-vesper/memory/skills/docx/scripts/docx_read.py out.docx --text
python ~/.agent-vesper/memory/skills/docx/scripts/docx_edit.py replace out.docx --find old --replace new
python ~/.agent-vesper/memory/skills/docx/scripts/docx_template.py tpl.docx values.json filled.docx
python ~/.agent-vesper/memory/skills/docx/scripts/docx_revisions.py list out.docx
python ~/.agent-vesper/memory/skills/docx/scripts/docx_comments.py list out.docx
python ~/.agent-vesper/memory/skills/docx/scripts/docx_validate.py out.docx
```
...
1. **Create.** Write a JSON spec with `write_file`, then run
   `~/.agent-vesper/memory/skills/docx/scripts/docx_create.py`. The spec supports: `page` (size + margins in
   ...
   format is documented at the top of `~/.agent-vesper/memory/skills/docx/scripts/docx_create.py`.
2. **Read.** Use `~/.agent-vesper/memory/skills/docx/scripts/docx_read.py` with exactly one mode flag.
...
3. **Edit.** Use `~/.agent-vesper/memory/skills/docx/scripts/docx_edit.py`. `replace` walks body, tables
...
6. **Template.** Put `{{name}}`-style tokens in the document. Run
   `~/.agent-vesper/memory/skills/docx/scripts/docx_template.py` with a JSON object of values. Use
...
- **Revision coverage.** `docx_revisions.py` resolves run-level
  ...
  `~/.agent-vesper/memory/skills/docx/references/revisions-and-comments.md` and hand those to Word.
...
- **Don't unzip-and-sed the XML.** Edit through the scripts (or
  ...
  easily. Use `apply_patch_set`/`write_file` only for the JSON inputs, never on the
  `.docx` itself.
...
> Bundled resources for this skill (~/.agent-vesper/memory/skills/docx/references/scripts/templates) live in `~/.agent-vesper/memory/skills/docx/` next to this file.
