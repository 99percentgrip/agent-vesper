...
version: 1.1.0
author: Agent Vesper library (adapted from obra/superpowers)
license: MIT
...
**Action:** Use the `bash` tool to run the tight loop:

```bash
# Run a specific failing test
pytest ~/.agent-vesper/memory/skills/systematic-debugging/tests/test_module.py::test_name -v

# Or run a scripted repro
python ~/.agent-vesper/memory/skills/systematic-debugging/scripts/repro_bug.py

# Or run a high-repetition flaky repro
for i in {1..100}; do pytest ~/.agent-vesper/memory/skills/systematic-debugging/tests/test_flake.py::test_name -q || break; done
```
...
pytest ~/.agent-vesper/memory/skills/systematic-debugging/tests/test_module.py::test_regression -v
...
Use these the agent's hosted tools during Phase 1:

- **`search_files`** — Find error strings, trace function calls, locate patterns
- **`read_file`** — Read source code with line numbers for precise analysis
- **`bash`** — Run tests, check git history, reproduce bugs
- **`web_search`/`web_reader`** — Research error messages, library docs
