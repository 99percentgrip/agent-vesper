...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the the agent's terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For agent hosts that route Codex as a provider, `model.provider: openai-codex` uses agent-managed Codex
OAuth from the Codex CLI's own credential store after `codex login`. For the
...
bash(command="codex exec --sandbox workspace-write 'Refactor the auth module'", workdir="~/project", background=true, pty=true)
# Returns session_id

> **Agent Vesper note:** this host runs CLIs through `bash`; prefer the
> non-interactive `codex exec` form above. The interactive session
> management below (submit/poll/log/kill actions) applies to hosts with a
> PTY-capable process tool.

# Monitor progress
bash("ps aux | grep codex")   # poll
...
When invoking the Codex CLI from an agent gateway/service context (for example,
...
bash(command="codex --sandbox workspace-write exec 'Fix issue #78: <description>. Commit when done.'", workdir="/tmp/issue-78", background=true, pty=true)
bash(command="codex --sandbox workspace-write exec 'Fix issue #99: <description>. Commit when done.'", workdir="/tmp/issue-99", background=true, pty=true)

# Monitor
process(action="list")

# After completion, push and create PRs
bash(command="cd /tmp/issue-78 && git push -u origin fix/issue-78")
bash(command="gh pr create --repo user/repo --head fix/issue-78 --title 'fix: ...' --body '...'")

# Cleanup
bash(command="git worktree remove /tmp/issue-78", workdir="~/project")
```
...
```
# Fetch all PR refs
bash(command="git fetch origin '+refs/pull/*/head:refs/remotes/origin/pr/*'", workdir="~/project")

# Review multiple PRs in parallel
bash(command="codex exec 'Review PR #86. git diff origin/main...origin/pr/86'", workdir="~/project", background=true, pty=true)
bash(command="codex exec 'Review PR #87. git diff origin/main...origin/pr/87'", workdir="~/project", background=true, pty=true)

# Post results
bash(command="gh pr comment 86 --body '<review>'", workdir="~/project")
```
