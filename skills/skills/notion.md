...
Add those exports to your shell profile (or to your shell profile`) so every session inherits them.
...
- Notion also ships an MCP server now (`Notion MCP`, ~91% more token-efficient on DB ops than the previous version) — wire it via the agent's MCP support if you want streaming Notion access from inside a session, but the paths above are enough for most one-shot tasks.

> Bundled resources for this skill (~/.agent-vesper/memory/skills/notion/references/scripts/templates) live in `~/.agent-vesper/memory/skills/notion/` next to this file.
