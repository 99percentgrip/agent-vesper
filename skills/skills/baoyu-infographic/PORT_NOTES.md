...
| Change | Upstream | Agent Vesper |
|--------|----------|--------|
| Metadata namespace | `openclaw` | `agent-vesper` |
| Trigger | `/baoyu-infographic` slash command | Natural language skill matching |
| User config | EXTEND.md file (project/user/XDG paths) | Removed — not part of Agent Vesper infra |
...
| File operations | Bash commands | Agent Vesper file tools (write_file, read_file) |
...
Reference files can be overwritten directly (they're unchanged from upstream). SKILL.md must be manually merged since it contains Agent Vesper-specific adaptations.
