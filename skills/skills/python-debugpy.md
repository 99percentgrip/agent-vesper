...
related_skills: [node-inspect-debugger]
---
...
- A long-running process (a gateway service, daemon) misbehaves and you can't restart it
...
The project test runner and pytest both support this:

```bash
# Drop to pdb on failure (or on any raised exception):
~/.agent-vesper/memory/skills/python-debugpy/scripts/run_tests.sh ~/.agent-vesper/memory/skills/python-debugpy/tests/path/to/test_file.py::test_name --pdb

# Drop to pdb at the START of the test:
~/.agent-vesper/memory/skills/python-debugpy/scripts/run_tests.sh ~/.agent-vesper/memory/skills/python-debugpy/tests/path/to/test_file.py::test_name --trace

# Show locals in tracebacks without pdb:
~/.agent-vesper/memory/skills/python-debugpy/scripts/run_tests.sh ~/.agent-vesper/memory/skills/python-debugpy/tests/path/to/test_file.py --showlocals --tb=long
```

Note: `~/.agent-vesper/memory/skills/python-debugpy/scripts/run_tests.sh` runs each test file in a captured subprocess via `run_tests_parallel.py` (no xdist), so interactive pdb does NOT work under the wrapper. Run pytest directly for `--pdb`:

```bash
source .venv/bin/activate
python -m pytest ~/.agent-vesper/memory/skills/python-debugpy/tests/foo_test.py::test_bar --pdb
```
...
For long-lived processes: a gateway service, a daemon, a process that's already misbehaving and can't be restarted clean.

### Setup

```bash
source <target-repo>/.venv/bin/activate
pip install debugpy
```
...
```json
{
  ...
  "justMyCode": false,
  "pathMappings": [
    { "localRoot": "${workspaceFolder}", "remoteRoot": "<target-repo>" }
  ]
}
```
...
## Debugging Agent-Specific Processes

### Tests
See Recipe 3. The wrapper captures subprocess output, so run pytest directly for interactive pdb.

### `run_agent.py` / CLI — one-shot
Easiest: add `breakpoint()` near the suspect line, then run the command normally. Control returns to your terminal at the pause point.

### `tui_gateway` subprocess (spawned by `agent-vesper-tui`)
...
debugpy.wait_for_client()
```
Start `agent-vesper-tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
1. **pdb under a parallel/output-capturing runner silently does nothing.** You won't see the prompt, the test just hangs (true of pytest-xdist and of `~/.agent-vesper/memory/skills/python-debugpy/scripts/run_tests.sh`'s captured per-file subprocesses). Run pytest directly on a single file for interactive debugging.
...
8. **`~/.agent-vesper/memory/skills/python-debugpy/scripts/run_tests.sh` strips credentials and sets `HOME=<tmpdir>`.** If your bug depends on user config or real API keys, it won't reproduce under the wrapper. Debug with raw `pytest` first to repro, then re-confirm under the wrapper.

9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For delegated subagents, debug one process at a time.
...
```bash
~/.agent-vesper/memory/skills/python-debugpy/scripts/run_tests.sh ~/.agent-vesper/memory/skills/python-debugpy/tests/the_test.py   # confirm it fails under the isolated runner first
