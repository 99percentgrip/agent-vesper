#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Agent Vesper.
...
def _agent_vesper_home() -> Path:
    return Path(
        os.environ.get("AGENT_VESPER_HOME", str(Path.home() / ".agent-vesper"))
    ).expanduser()


AV_HOME = _agent_vesper_home()
TOKEN_PATH = AV_HOME / "google_token.json"
CLIENT_SECRET_PATH = AV_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = AV_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Agent Vesper profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (container images whose venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or "
        "a uv-managed venv without pip), install manually:"
    )
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Agent Vesper home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {_agent_vesper_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Agent Vesper")
