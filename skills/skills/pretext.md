...
related_skills: [claude-design, excalidraw, architecture-diagram]
---
...
This skill exists so the agent can make **cool demos** with it — the kind people post to X. See `pretext.cool` and `chenglou.me/pretext` for the community demo corpus.
...
The community corpus (see `~/.agent-vesper/memory/skills/pretext/references/patterns.md`) clusters into a handful of strong patterns. Pick one and riff — don't invent a new category unless asked.
...
See `~/.agent-vesper/memory/skills/pretext/templates/donut-orbit.html` and `~/.agent-vesper/memory/skills/pretext/templates/hello-orb-flow.html` for working single-file starters.

## Workflow

1. **Pick a pattern** from the table above based on the user's brief.
2. **Start from a template**:
   - `~/.agent-vesper/memory/skills/pretext/templates/hello-orb-flow.html` — text reflowing around a moving orb (reflow-around-obstacle pattern)
   - `~/.agent-vesper/memory/skills/pretext/templates/donut-orbit.html` — advanced example: measured ASCII logo obstacles, draggable wire sphere/cube, morphing shape fields, selectable DOM text, and dev-only controls
...
> Bundled resources for this skill (~/.agent-vesper/memory/skills/pretext/references/scripts/templates) live in `~/.agent-vesper/memory/skills/pretext/` next to this file.
