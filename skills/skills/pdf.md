...
related_skills: [xlsx, powerpoint, ocr-and-documents]
---
...
```bash
python ~/.agent-vesper/memory/skills/pdf/scripts/pdf_create.py spec.json -o out.pdf         # build PDF from JSON spec
python ~/.agent-vesper/memory/skills/pdf/scripts/pdf_make_form.py formspec.json -o form.pdf # build fillable AcroForm from JSON spec
python ~/.agent-vesper/memory/skills/pdf/scripts/pdf_form_layout.py formspec.json           # lint form layout BEFORE building
python ~/.agent-vesper/memory/skills/pdf/scripts/pdf_form_layout.py formspec.json --render-overlay boxes.png [--pdf form.pdf]
python ~/.agent-vesper/memory/skills/pdf/scripts/pdf_read.py doc.pdf --text                 # per-page text (JSON)
python ~/.agent-vesper/memory/skills/pdf/scripts/pdf_read.py doc.pdf --tables --csv-dir t/  # tables to JSON + CSV files
python ~/.agent-vesper/memory/skills/pdf/scripts/pdf_read.py doc.pdf --meta                 # metadata, page sizes, encrypted/scanned flags
python ~/.agent-vesper/memory/skills/pdf/scripts/pdf_read.py form.pdf --fields              # form fields: name, type, value
python ~/.agent-vesper/memory/skills/pdf/scripts/pdf_merge.py a.pdf b.pdf -o merged.pdf [--bookmarks]
python ~/.agent-vesper/memory/skills/pdf/scripts/pdf_split.py doc.pdf --pages 1-3,7 -o part.pdf [--rotate 90]
python ~/.agent-vesper/memory/skills/pdf/scripts/pdf_fill_form.py form.pdf --fields-json values.json -o filled.pdf [--flatten]
python ~/.agent-vesper/memory/skills/pdf/scripts/pdf_secure.py doc.pdf --encrypt -o enc.pdf --user-password your-password
python ~/.agent-vesper/memory/skills/pdf/scripts/pdf_secure.py enc.pdf --decrypt -o dec.pdf --password your-password
python ~/.agent-vesper/memory/skills/pdf/scripts/pdf_watermark.py doc.pdf --stamp mark.pdf -o stamped.pdf [--under]
python ~/.agent-vesper/memory/skills/pdf/scripts/pdf_stamp.py doc.pdf -o out.pdf --text "DRAFT" --x 150 --y 400 \
    --font-size 60 --rotation 45 --opacity 0.3 --color "#cc0000" [--pages 1-3]
python ~/.agent-vesper/memory/skills/pdf/scripts/pdf_stamp.py doc.pdf -o out.pdf --image sig.png --x 400 --y 60 --width 120
python ~/.agent-vesper/memory/skills/pdf/scripts/pdf_page_image.py doc.pdf --pages 1-3 --dpi 150 --out-dir imgs/
python ~/.agent-vesper/memory/skills/pdf/scripts/pdf_meta.py doc.pdf --set-meta --title "T" --author "A" -o out.pdf
python ~/.agent-vesper/memory/skills/pdf/scripts/pdf_meta.py doc.pdf --attach data.csv -o out.pdf
python ~/.agent-vesper/memory/skills/pdf/scripts/pdf_meta.py doc.pdf --list-attachments | --extract-attachments dir/
```
...
5. **Build forms.** Write one form-spec JSON (fields with `label_box`/`entry_box` in PDF points — see `~/.agent-vesper/memory/skills/pdf/references/forms.md`), lint it with `pdf_form_layout.py` and fix every reported problem, optionally review the `--render-overlay` PNG with `vision_analyze`, then build with `pdf_make_form.py` and confirm with `pdf_read.py --fields`.
...
- **Radio groups**: reportlab needs ≥2 `radio()` widgets per group, fills need the slashed export value (`"/red"`), and flatten fidelity is worst for radios — see `~/.agent-vesper/memory/skills/pdf/references/forms.md`.
...
> Bundled resources for this skill (~/.agent-vesper/memory/skills/pdf/references/scripts/templates) live in `~/.agent-vesper/memory/skills/pdf/` next to this file.
