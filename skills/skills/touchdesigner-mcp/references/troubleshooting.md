...
## 5. Agent Vesper Configuration

### Config location

`$AGENT_VESPER_HOME/config.yaml` (defaults to `~/.agent-vesper/config.yaml` when `AGENT_VESPER_HOME` is unset)
...
Restart the Agent Vesper session for changes to take effect. The MCP connection is
