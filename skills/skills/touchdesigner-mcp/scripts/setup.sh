...
AV_HOME_DIR="${AGENT_VESPER_HOME:-$HOME/.agent-vesper}"
AV_CFG="${AV_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Agent Vesper config has twozero_td MCP entry ──
if [[ ! -f "$AV_CFG" ]]; then
    echo -e " ${FAIL} Agent Vesper config not found at ${AV_CFG}"
    manual_steps+=("Create ${AV_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$AV_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Agent Vesper config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Agent Vesper config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${AV_CFG} manually"); }
    manual_steps+=("Restart Agent Vesper session to pick up config change")
fi
