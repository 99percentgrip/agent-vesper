...
Work with Airtable's REST API directly via `curl` using the `bash` tool. No MCP server, no OAuth flow, no Python SDK — just `curl` and a personal access token.
...
4. Store the token in your shell profile (e.g. `~/.bashrc`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for the agent. Pipe through `python -m json.tool` (always present) or `jq` (if installed) for readable JSON.
...
- **Always use the `bash` tool with `curl`.** Do NOT use `web_reader` (it can't send auth headers) or `browser_navigate` (needs UI auth and is slow).
- **`AIRTABLE_API_KEY` flows from your shell profile` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
