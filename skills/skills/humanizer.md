...
version: 2.5.1
author: Siqi Chen (@blader, https://github.com/blader/humanizer), ported by Agent Vesper library
license: MIT
...
Also apply this skill to **your own** output when writing user-facing prose such as release notes, PR descriptions, docs, and summaries. the agent'ss baseline voice already strips most of these, but a focused pass catches what slips through.

## How to use it in the agent session

The text usually arrives one of three ways:
1. **Inline.** The user pastes the text into the message. Work on it in place and reply with the rewrite.
2. **File.** The user points at a file. Use `read_file` to load it, then `apply_patch_set` or `write_file` to apply edits. For a markdown doc in a repo, a targeted `apply_patch_set` per section is cleaner than rewriting the whole file.
...
10. If the text came from a file, apply the edit with `apply_patch_set` (targeted) or `write_file` (full rewrite) and show the user what changed.
