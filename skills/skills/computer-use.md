...
version: 2.0.0
author: Francesco Bonacci (f-trycua), Agent Vesper library
license: MIT
...
The agent drives [cua-driver](https://github.com/trycua/cua) under the hood.
This wrapper skill teaches the cua-driver `computer_use` workflow and action
...
skill installed by `cua-driver skills install`. Automatic detection is a
planned cua-driver follow-up, so currently point the agent at the resulting
...
1. **Config grant (standard and unrestricted modes).** When
   ...
   the host applies the same floor in unrestricted mode. If it is not
...
Explicit unrestricted host mode (`approvals.mode: off` on the host runtime) launches an
...
|---|---|
| `cua-driver not installed` | Install cua-driver from its repo README and enable its Computer Use integration |
| Captures consistently return empty / "no on-screen window" | On Linux: DISPLAY may not be set (X11) or you're on pure Wayland — ask the user to run the cua-driver diagnostics from its repo README. On Windows: you may be in Session 0 (SSH session) instead of the interactive desktop — see the cua-driver `WINDOWS.md` deep-dive |
...
| Type text disappears into a terminal emulator | cua-driver detects terminals (Ghostty, iTerm2, Terminal.app, Windows Terminal, mintty, etc.) and routes through key-event synthesis — should "just work" on a recent cua-driver. If it doesn't, ask the user to run the cua-driver diagnostics from its repo README |
...
| Anything else weird | **First action: ask the user to run the cua-driver diagnostics from its repo README.** It runs the cua-driver `health_report` MCP tool and prints a structured per-check matrix. Their output tells you (and them) exactly what's wrong |

## When NOT to use `computer_use`

- **Web content fetching** — use `web_reader` / `web_search` or `bash` (curl)
  instead of driving the user's GUI browser. Reach for `computer_use` specifically when the task
  needs the user's actual native apps (Finder/Explorer/Files, Mail/
  Outlook/Thunderbird, native chat clients, Figma, Logic, games,
  anything non-web).
- **File edits** — use `read_file` / `write_file` / `apply_patch_set`, not
...
This skill intentionally stays focused on the agent-side
...
Automatic detection is a planned follow-up in trycua/cua. For now, the command
installs the pack under `~/.cua-driver/skills/cua-driver`; point the agent at that
