...
version: 1.0.0
author: SHL0MS, Agent Vesper library
license: MIT
...
|------|-------|--------|-----------|
| **Generative art** | Seed / parameters | Procedural visual composition (still or animated) | `~/.agent-vesper/memory/skills/p5js/references/visual-effects.md` |
| **Data visualization** | Dataset / API | Interactive charts, graphs, custom data displays | `~/.agent-vesper/memory/skills/p5js/references/interaction.md` |
| **Interactive experience** | None (user drives) | Mouse/keyboard/touch-driven sketch | `~/.agent-vesper/memory/skills/p5js/references/interaction.md` |
| **Animation / motion graphics** | Timeline / storyboard | Timed sequences, kinetic typography, transitions | `~/.agent-vesper/memory/skills/p5js/references/animation.md` |
| **3D scene** | Concept description | WebGL geometry, lighting, camera, materials | `~/.agent-vesper/memory/skills/p5js/references/webgl-and-3d.md` |
| **Image processing** | Image file(s) | Pixel manipulation, filters, mosaic, pointillism | `~/.agent-vesper/memory/skills/p5js/references/visual-effects.md` § Pixel Manipulation |
| **Audio-reactive** | Audio file / mic | Sound-driven generative visuals | `~/.agent-vesper/memory/skills/p5js/references/interaction.md` § Audio Input |
...
**p5.js 2.x** (2.2+) adds: `async setup()` replacing `preload()`, OKLCH/OKLAB color modes, `splineVertex()`, shader `.modify()` API, variable fonts, `textToContours()`, pointer events. Required for p5.brush. See `~/.agent-vesper/memory/skills/p5js/references/core-api.md` § p5.js 2.0.
...
|-----------|---------|-----------|
| **Color system** | HSB/HSL, RGB, named palettes, procedural harmony, gradient interpolation | `~/.agent-vesper/memory/skills/p5js/references/color-systems.md` |
| **Noise vocabulary** | Perlin noise, simplex, fractal (octaved), domain warping, curl noise | `~/.agent-vesper/memory/skills/p5js/references/visual-effects.md` § Noise |
| **Particle systems** | Physics-based, flocking, trail-drawing, attractor-driven, flow-field following | `~/.agent-vesper/memory/skills/p5js/references/visual-effects.md` § Particles |
| **Shape language** | Geometric primitives, custom vertices, bezier curves, SVG paths | `~/.agent-vesper/memory/skills/p5js/references/shapes-and-geometry.md` |
| **Motion style** | Eased, spring-based, noise-driven, physics sim, lerped, stepped | `~/.agent-vesper/memory/skills/p5js/references/animation.md` |
| **Typography** | System fonts, loaded OTF, `textToPoints()` particle text, kinetic | `~/.agent-vesper/memory/skills/p5js/references/typography.md` |
| **Shader effects** | GLSL fragment/vertex, filter shaders, post-processing, feedback loops | `~/.agent-vesper/memory/skills/p5js/references/webgl-and-3d.md` § Shaders |
| **Composition** | Grid, radial, golden ratio, rule of thirds, organic scatter, tiled | `~/.agent-vesper/memory/skills/p5js/references/core-api.md` § Composition |
| **Interaction model** | Mouse follow, click spawn, drag, keyboard state, scroll-driven, mic input | `~/.agent-vesper/memory/skills/p5js/references/interaction.md` |
| **Blend modes** | `BLEND`, `ADD`, `MULTIPLY`, `SCREEN`, `DIFFERENCE`, `EXCLUSION`, `OVERLAY` | `~/.agent-vesper/memory/skills/p5js/references/color-systems.md` § Blend Modes |
| **Layering** | `createGraphics()` offscreen buffers, alpha compositing, masking | `~/.agent-vesper/memory/skills/p5js/references/core-api.md` § Offscreen Buffers |
| **Texture** | Perlin surface, stippling, hatching, halftone, pixel sorting | `~/.agent-vesper/memory/skills/p5js/references/visual-effects.md` § Texture Generation |
...
- **Viewer UI** — for interactive generative art, start from `~/.agent-vesper/memory/skills/p5js/templates/viewer.html` which provides seed navigation, parameter sliders, and download. For simple sketches or video export, use bare HTML

### Step 3: Code the Sketch

For **interactive generative art** (seed exploration, parameter tuning): start from `~/.agent-vesper/memory/skills/p5js/templates/viewer.html`. Read the template first, keep the fixed sections (seed nav, actions), replace the algorithm and parameter controls. This gives the user seed prev/next/random/jump, parameter sliders with live update, and PNG download — all wired up.
...
- For `loadImage()`/`loadFont()` from local files: use `~/.agent-vesper/memory/skills/p5js/scripts/serve.sh` or `python -m http.server`
...
| **High-res PNG** | Puppeteer headless capture | `node ~/.agent-vesper/memory/skills/p5js/scripts/export-frames.js sketch.html --width 3840 --height 2160 --frames 1` |
...
| **MP4** | Puppeteer frame capture + ffmpeg | `bash ~/.agent-vesper/memory/skills/p5js/scripts/render.sh sketch.html output.mp4 --duration 30 --fps 30` |
...
Never `console.log()` inside `draw()`. Never manipulate DOM in `draw()`. See `~/.agent-vesper/memory/skills/p5js/references/troubleshooting.md` § Performance.
...
See `~/.agent-vesper/memory/skills/p5js/references/export-pipeline.md` § Platform Export.
...
Never hardcode raw RGB values. Define a palette object, derive variations procedurally. See `~/.agent-vesper/memory/skills/p5js/references/color-systems.md`.
...
For flowing organic forms, use **domain warping**: feed noise output back as noise input coordinates. See `~/.agent-vesper/memory/skills/p5js/references/visual-effects.md`.
...
See `~/.agent-vesper/memory/skills/p5js/references/troubleshooting.md` § Performance.
...
The bundled `~/.agent-vesper/memory/skills/p5js/scripts/export-frames.js` detects `_p5Ready` and calls `redraw()` once per capture for exact 1:1 frame correspondence. See `~/.agent-vesper/memory/skills/p5js/references/export-pipeline.md` § Deterministic Capture.

For multi-scene videos, use the per-clip architecture: one HTML per scene, render independently, stitch with `ffmpeg -f concat`. See `~/.agent-vesper/memory/skills/p5js/references/export-pipeline.md` § Per-Clip Architecture.
...
5. **Headless export** — `node ~/.agent-vesper/memory/skills/p5js/scripts/export-frames.js sketch.html --frames 300` for automated frame capture (sketch must use `noLoop()` + `_p5Ready`)
6. **MP4 rendering** — `bash ~/.agent-vesper/memory/skills/p5js/scripts/render.sh sketch.html output.mp4 --duration 30`
7. **Iterative refinement** — edit the HTML file, user refreshes browser to see changes
8. **Load references on demand** — use `read_skill(name=name="p5js", file_path="~/.agent-vesper/memory/skills/p5js/references/...")` to load specific reference files as needed during implementation
...
| File | Contents |
|------|----------|
| `~/.agent-vesper/memory/skills/p5js/references/core-api.md` | Canvas setup, coordinate system, draw loop, `push()`/`pop()`, offscreen buffers, composition patterns, `pixelDensity()`, responsive design |
| `~/.agent-vesper/memory/skills/p5js/references/shapes-and-geometry.md` | 2D primitives, `beginShape()`/`endShape()`, Bezier/Catmull-Rom curves, `vertex()` systems, custom shapes, `p5.Vector`, signed distance fields, SVG path conversion |
| `~/.agent-vesper/memory/skills/p5js/references/visual-effects.md` | Noise (Perlin, fractal, domain warp, curl), flow fields, particle systems (physics, flocking, trails), pixel manipulation, texture generation (stipple, hatch, halftone), feedback loops, reaction-diffusion |
| `~/.agent-vesper/memory/skills/p5js/references/animation.md` | Frame-based animation, easing functions, `lerp()`/`map()`, spring physics, state machines, timeline sequencing, `millis()`-based timing, transition patterns |
| `~/.agent-vesper/memory/skills/p5js/references/typography.md` | `text()`, `loadFont()`, `textToPoints()`, kinetic typography, text masks, font metrics, responsive text sizing |
| `~/.agent-vesper/memory/skills/p5js/references/color-systems.md` | `colorMode()`, HSB/HSL/RGB, `lerpColor()`, `paletteLerp()`, procedural palettes, color harmony, `blendMode()`, gradient rendering, curated palette library |
| `~/.agent-vesper/memory/skills/p5js/references/webgl-and-3d.md` | WEBGL renderer, 3D primitives, camera, lighting, materials, custom geometry, GLSL shaders (`createShader()`, `createFilterShader()`), framebuffers, post-processing |
| `~/.agent-vesper/memory/skills/p5js/references/interaction.md` | Mouse events, keyboard state, touch input, DOM elements, `createSlider()`/`createButton()`, audio input (p5.sound FFT/amplitude), scroll-driven animation, responsive events |
| `~/.agent-vesper/memory/skills/p5js/references/export-pipeline.md` | `saveCanvas()`, `saveGif()`, `saveFrames()`, deterministic headless capture, ffmpeg frame-to-video, CCapture.js, SVG export, per-clip architecture, platform export (fxhash), video gotchas |
| `~/.agent-vesper/memory/skills/p5js/references/troubleshooting.md` | Performance profiling, per-pixel budgets, common mistakes, browser compatibility, WebGL debugging, font loading issues, pixel density traps, memory leaks, CORS |
| `~/.agent-vesper/memory/skills/p5js/templates/viewer.html` | Interactive viewer template: seed navigation (prev/next/random/jump), parameter sliders, download PNG, responsive canvas. Start from this for explorable generative art |
...
> Bundled resources for this skill (~/.agent-vesper/memory/skills/p5js/references/scripts/templates) live in `~/.agent-vesper/memory/skills/p5js/` next to this file.
