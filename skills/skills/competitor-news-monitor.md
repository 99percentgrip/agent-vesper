...
Don't use for: one-off company research (use `web_search`/`web_reader` directly) or plain feed reading (`blogwatcher`).
...
Use `blogwatcher` for feeds and `web_search`/`web_reader` for pages. Write the watch contract (watchlist, categories, materiality threshold, last cutoff) to a state file under `~/.agent-vesper/competitor-watches/<watch-slug>.json`, then create the job:

```
cronjob(action="create",
        schedule="every monday 9am",
        prompt="Load the competitor-news-monitor skill and run the tick for the watch contract at ~/.agent-vesper/competitor-watches/<watch-slug>.json.",
