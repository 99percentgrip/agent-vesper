...
|------|-------|--------|-----------|
| **Video-to-ASCII** | Video file | ASCII recreation of source footage | `~/.agent-vesper/memory/skills/ascii-video/references/inputs.md` § Video Sampling |
| **Audio-reactive** | Audio file | Generative visuals driven by audio features | `~/.agent-vesper/memory/skills/ascii-video/references/inputs.md` § Audio Analysis |
| **Generative** | None (or seed params) | Procedural ASCII animation | `~/.agent-vesper/memory/skills/ascii-video/references/effects.md` |
| **Hybrid** | Video + audio | ASCII video with audio-reactive overlays | Both input refs |
| **Lyrics/text** | Audio + text/SRT | Timed text with visual effects | `~/.agent-vesper/memory/skills/ascii-video/references/inputs.md` § Text/Lyrics |
| **TTS narration** | Text quotes + TTS API | Narrated testimonial/quote video with typed text | `~/.agent-vesper/memory/skills/ascii-video/references/inputs.md` § TTS Integration |
...
3. **SCENE_FN** — Scene function renders to pixel canvas (`uint8 H,W,3`). Composes multiple character grids via `_render_vf()` + pixel blend modes. See `~/.agent-vesper/memory/skills/ascii-video/references/composition.md`
4. **TONEMAP** — Percentile-based adaptive brightness normalization. See `~/.agent-vesper/memory/skills/ascii-video/references/composition.md` § Adaptive Tonemap
5. **SHADE** — Post-processing via `ShaderChain` + `FeedbackBuffer`. See `~/.agent-vesper/memory/skills/ascii-video/references/shaders.md`
...
- **Hardware detection** — auto-detect cores/RAM, set quality profile. See `~/.agent-vesper/memory/skills/ascii-video/references/optimization.md`
...
1. **Hardware detection + quality profile** — `~/.agent-vesper/memory/skills/ascii-video/references/optimization.md`
2. **Input loader** — mode-dependent; `~/.agent-vesper/memory/skills/ascii-video/references/inputs.md`
3. **Feature analyzer** — audio FFT, video luminance, or synthetic
4. **Grid + renderer** — multi-density grids with bitmap cache; `~/.agent-vesper/memory/skills/ascii-video/references/architecture.md`
5. **Character palettes** — multiple per project; `~/.agent-vesper/memory/skills/ascii-video/references/architecture.md` § Palettes
6. **Color system** — HSV + discrete RGB + harmony generation; `~/.agent-vesper/memory/skills/ascii-video/references/architecture.md` § Color
7. **Scene functions** — each returns `canvas (uint8 H,W,3)`; `~/.agent-vesper/memory/skills/ascii-video/references/scenes.md`
8. **Tonemap** — adaptive brightness normalization; `~/.agent-vesper/memory/skills/ascii-video/references/composition.md`
9. **Shader pipeline** — `ShaderChain` + `FeedbackBuffer`; `~/.agent-vesper/memory/skills/ascii-video/references/shaders.md`
10. **Scene table + dispatcher** — time → scene function + config; `~/.agent-vesper/memory/skills/ascii-video/references/scenes.md`
...
macOS Pillow: `textbbox()` returns wrong height. Use `font.getmetrics()`: `cell_height = ascent + descent`. See `~/.agent-vesper/memory/skills/ascii-video/references/troubleshooting.md`.

### ffmpeg Pipe Deadlock

Never `stderr=subprocess.PIPE` with long-running ffmpeg — buffer fills at 64KB and deadlocks. Redirect to file. See `~/.agent-vesper/memory/skills/ascii-video/references/troubleshooting.md`.

### Font Compatibility

Not all Unicode chars render in all fonts. Validate palettes at init — render each char, check for blank output. See `~/.agent-vesper/memory/skills/ascii-video/references/troubleshooting.md`.

### Per-Clip Architecture

For segmented videos (quotes, scenes, chapters), render each as a separate clip file for parallel rendering and selective re-rendering. See `~/.agent-vesper/memory/skills/ascii-video/references/scenes.md`.
...
| File | Contents |
|------|----------|
| `~/.agent-vesper/memory/skills/ascii-video/references/architecture.md` | Grid system, resolution presets, font selection, character palettes (20+), color system (HSV + OKLAB + discrete RGB + harmony generation), `_render_vf()` helper, GridLayer class |
| `~/.agent-vesper/memory/skills/ascii-video/references/composition.md` | Pixel blend modes (20 modes), `blend_canvas()`, multi-grid composition, adaptive `tonemap()`, `FeedbackBuffer`, `PixelBlendStack`, masking/stencil system |
| `~/.agent-vesper/memory/skills/ascii-video/references/effects.md` | Effect building blocks: value field generators, hue fields, noise/fBM/domain warp, voronoi, reaction-diffusion, cellular automata, SDFs, strange attractors, particle systems, coordinate transforms, temporal coherence |
| `~/.agent-vesper/memory/skills/ascii-video/references/shaders.md` | `ShaderChain`, `_apply_shader_step()` dispatch, 38 shader catalog, audio-reactive scaling, transitions, tint presets, output format encoding, terminal rendering |
| `~/.agent-vesper/memory/skills/ascii-video/references/scenes.md` | Scene protocol, `Renderer` class, `SCENES` table, `render_clip()`, beat-synced cutting, parallel rendering, design patterns (layer hierarchy, directional arcs, visual metaphors, compositional techniques), complete scene examples at every complexity level, scene design checklist |
| `~/.agent-vesper/memory/skills/ascii-video/references/inputs.md` | Audio analysis (FFT, bands, beats), video sampling, image conversion, text/lyrics, TTS integration (ElevenLabs, voice assignment, audio mixing) |
| `~/.agent-vesper/memory/skills/ascii-video/references/optimization.md` | Hardware detection, quality profiles, vectorized patterns, parallel rendering, memory management, performance budgets |
| `~/.agent-vesper/memory/skills/ascii-video/references/troubleshooting.md` | NumPy broadcasting traps, blend mode pitfalls, multiprocessing/pickling, brightness diagnostics, ffmpeg issues, font problems, common mistakes |
...
> Bundled resources for this skill (~/.agent-vesper/memory/skills/ascii-video/references/scripts/templates) live in `~/.agent-vesper/memory/skills/ascii-video/` next to this file.
