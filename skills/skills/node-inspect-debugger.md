...
Note: `chrome-remote-interface` is NOT in the target app's `package.json`. Install it to a throwaway location if you don't want to dirty the project:
...
### Debugging a running `agent-vesper-tui`
...
6. **Running `node inspect` through an agent terminal.** It's a PTY-friendly REPL. Launch it through `bash`; interactive stepping needs a real terminal — run one-shot commands non-interactively and drive stepping from the user's terminal when needed.
