...
version: 1.1.0
author: Jakub Wolniewicz (@frizikk) + Agent Vesper library
license: MIT
...
- Workspace access through `read_file`, `search_files`, and `bash` when the deliverable is code.
...
| 2 | Execution | Check out the work and actually run it via `bash`: build, test, and exercise the reported behavior yourself. Verify each handoff claim empirically instead of re-reading the artifact. |
...
2. Use `bash` to inspect the diff and run the project's existing focused tests, lint, type checks, or build commands.
