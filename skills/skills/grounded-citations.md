...
feeds (see `~/.agent-vesper/memory/skills/grounded-citations/references/citation-formats.md`).
...
None beyond the standard toolset. `~/.agent-vesper/memory/skills/grounded-citations/scripts/sources.py` is stdlib-only Python 3.
Retrieval comes from whatever is configured: `web_search`, `web_reader`,
or `bash` (curl, CLIs).

Ledger location: `~/.agent-vesper/cache/citations/ledger.json` (profile-aware).
Override per task with `--ledger <path>` or `AGENT_VESPER_CITATION_LEDGER`.

## How to Run

```bash
S=~/.agent-vesper/memory/skills/grounded-citations/scripts/sources.py
...
`web_reader` / `browser_navigate` / fetch, pass the URLs to `sources.py add`
...
`~/.agent-vesper/memory/skills/grounded-citations/references/citation-formats.md` for placement (footnotes in docx, endnotes in
...
- **Citing a search snippet as if you read the page.** A `web_search`
  ...
  when the claim needs the body — `web_reader` it first.
...
- **Parallel subagents.** Each subagent has its own working directory; point
  them all at one ledger with `--ledger` (or `AGENT_VESPER_CITATION_LEDGER`) if their
  outputs get merged, otherwise their ids will collide.
- **Quoting from a snippet instead of the page.** Evidence quotes must come
  from the extracted page text, not a search-result description — `web_reader`
...
> Bundled resources for this skill (~/.agent-vesper/memory/skills/grounded-citations/references/scripts/templates) live in `~/.agent-vesper/memory/skills/grounded-citations/` next to this file.
