...
version: 1.1.0
author: Agent Vesper library (inspired by Claude Code /simplify)
license: MIT
...
wider codebase. Each reviewer gets `bash`, `file`, and `search`
...
- **Respect project conventions.** If the repo has AGENTS.md / CLAUDE.md /
  AGENTS.md or a linter config, fold those rules into the reviewer prompts so
...
If delegating via `delegate_task` (optional), it
